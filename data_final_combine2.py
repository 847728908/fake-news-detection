...
def preprocess_and_save_data(base_dir, num_train, num_val, num_test):
    ...
    for name, files in datasets.items():
        ...
        save_path = Path(f'./{name}_data.pt')
...
def main():
    ...
    train_path = Path('./train_data.pt')
    val_path = Path('./validation_data.pt')
    ...
    best_model_path = Path('best_fusion_model.pth')
...
# --- 测试集最终评估结果 ---
# 测试集损失 (Loss): 0.5962
# 测试集准确率 (Accuracy): 0.8000
# 测试集 F1-Score (weighted): 0.7111
# 测试集精确率 (Precision, weighted): 0.6400
# 测试集召回率 (Recall, weighted): 0.8000