...
def determine_model_verdict_and_sentiment(response):
    ...
    if "假新闻" in last_line:
        ...
        if "假新闻" in response:
            ...
            result['verdict'] = "real"
        elif "虚假" in response or "不可信" in response or "误导" in response:
            result['verdict'] = "fake"
    ...
    elif "消极" in last_line:
        result['sentiment'] = "negative"
    elif "中性" in last_line:
...
def main():
    ...
    result_file = Path(r"C:\Users\84772\Desktop\try\result2_combine_text_newprompt.txt")
    result_real_file = Path(r"C:\Users\84772\Desktop\try\result_real2_combine_text_newprompt.txt")
    result_sentiment_file = Path(r"C:\Users\84772\Desktop\try\result_sentiment2_combine_text_newprompt.txt")  # 新增情感分析结果文件
    ...
    for file_path in supported_files:
        ...
        # 拼接 post_text
        post_text = ""
        if file_name_no_ext in file_name_info and "post_text" in file_name_info[file_name_no_ext]:
            post_text = file_name_info[file_name_no_ext]["post_text"]
        if post_text:
            # 合并内容，优先 post_text，再加图片描述/网页内容
            content_for_model = f"原始推文内容：{post_text}\n\n附加内容：{content_for_model}" if content_for_model else f"原始推文内容：{post_text}"
